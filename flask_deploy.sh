...
#
rm -rf ~/mission_checker/src/check/; cd ~/a3/; git archive --format=tar HEAD | (mkdir ~/mission_checker/src/check && cd ~/mission_checker/src/check && tar xf -); cd ~/mission_checker/; chmod -R u+x src/check/check.sh
